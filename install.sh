...
EXIT_MSG="You have left from OSS Installer!" 
KEY_MAP=
...
# if there are mirrors add by OSS Linux,delete it
line=$(grep -n "Add by OSS Linux" /etc/pacman.d/mirrorlist | cut -d ":" -f 1)
if [ -z "$line" ]
then
    # do nothing
    echo
...
# add comment
sed -i "0,/$pattern/s/$pattern/## Add by OSS Linux,for $region\n&/" /etc/pacman.d/mirrorlist
...
# delete old config
last_line=$(grep -n "Add by OSS Linux" /mnt/etc/locale.gen | cut -d ":" -f 1 | head -1)
...
echo "## Add by OSS Linux">>/mnt/etc/locale.gen
...
echo "## Add by OSS Linux">/mnt/etc/locale.conf
...
echo "## Add by OSS Linux" >> /mnt/etc/sudoers 
...
$CHROOT git clone https://github.com/genemator/Archlinux-Installer.git /home/$usrname/Archlinux-Installer
git clone git@github.com:genemator/Archlinux-Installer.me.git /mnt/home/$usrname/Archlinux-Installer
